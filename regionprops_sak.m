function varargout = regionprops_sak( varargin )
% modified version of regionprops for extended connected components and data visualization
% CC = regionprops_sak(BW)
% CC = regionprops_sak(CC)
% CC = regionprops_sak(L)
% C = regionprops_sak(___,I)
...
if ~isempty( CData )
    interior = ( any( CData, 1 ) & any( CData, 2 ) );
    PixelIdxRim = find( bwmorph( interior, 'remove' ) );
    CC.Edge = false( size( CC.PixelIdxList ) );
else
    PixelIdxRim = [];
end
...
for i=1:CC.NumObjects
    ...
    [CC.rows{count}, CC.cols{count}] = ind2sub( CC.ImageSize, CC.PixelIdxList{count} );
    ...
    B = builtin("_bwtraceboundarymex", bw, [CC.rows{count}(end) + 1 - rlim(1), CC.cols{count}(end) + 1 - clim(1)], 8 , 0, numel( CC.PixelIdxList{count} ), 0);
    CC.Area(count) = numel( CC.PixelIdxList{count} );
    CC.Perimeter(count) = sum( sqrt( sum( diff(B).^2, 2 ) ) );
    if CC.Perimeter(count) > cut_off.Perimeter && numel( CC.PixelIdxList{count} ) > cut_off.Area
        %     cla; imagesc( bw ); hold on; plot( B(:, 2 ), B(:, 1 ), 'or' );
        CC.PixelIdxListContour{count} = uint32( sub2ind( CC.ImageSize, B(:,1) + rlim(1) - 1, B(:,2) + clim(1) - 1 ) );
        CC.PixelIdxFirst(count) = CC.PixelIdxList{count}(1);
        CC.PixelIdxLast(count) = CC.PixelIdxList{count}(end);
        %%
        ...
        %%
        xx = sum( double( CC.cols{count} - CC.Centroid(1, count ) ).^2 );
        yy = sum( double( CC.rows{count} - CC.Centroid(2, count ) ).^2 );
        xy = sum( double( CC.rows{count} - CC.Centroid(2, count ) ).*double( CC.cols{count} - CC.Centroid(1, count ) ) );
        
        [V,D] = eig([ xx xy; xy yy]+eps(1));
        if D(1) > D(4)
        ...
        tmp = [CC.cols{count} CC.rows{count}]*MajorAxis;
        CC.Length(count) = max(tmp) - min(tmp);
        tmp = [CC.cols{count} CC.rows{count}]*[MajorAxis(2), -MajorAxis(1)]';
        ...
        if PixelTolerance > 0
            ...
            CC.PixelIdxListSwath{count} = unique( uint32( sub2ind( CC.ImageSize, y(good), x(good) ) ) );
        end
        if ~bw_input
            [CC.level(count), level( CC.PixelIdxList{count} )] = deal( max( level( CC.PixelIdxList{count} ) ) + 1 );
        end
        
        if ~isempty( CData )
            %%
            CData = single( CData );
            CC.MeanIntensity(count) = mean( CData( CC.PixelIdxList{count} ) );
            CC.StdIntensity(count) = std( CData( CC.PixelIdxList{count} ) );
            CC.MaxIntensity(count) = max( CData( CC.PixelIdxList{count} ) );
            CC.MinIntensity(count) = min( CData( CC.PixelIdxList{count} ) );
            CC.MeanEdgeIntensity(count) = mean( CData( CC.PixelIdxListContour{count} ) );
            CC.StdEdgeIntensity(count) = std( CData( CC.PixelIdxListContour{count} ) );
            CC.MaxEdgeIntensity(count) = max( CData( CC.PixelIdxListContour{count} ) );
            CC.MinEdgeIntensity(count) = min( CData( CC.PixelIdxListContour{count} ) );   
            CC.Edge(count) = any( ismember( CC.PixelIdxListContour{count}, PixelIdxRim ) );
        end
    else
        CC.rows{count} = [];
        CC.cols{count} = [];
...
CC.NormalizedPerimeter = CC.Perimeter./sqrt( CC.Area );
...
if nargout == 0 || any( strncmpi( varargin, 'plot', 1 ) )
    ...
    if ~isempty( fig.UserData.CData )
        imagesc( ax.image, fig.UserData.CData, 'ButtonDownFcn', @ButtonDownFcn, 'UserData', labelmatrix( fig.UserData.CC ) );
    else
        imagesc( ax.image, labelmatrix( fig.UserData.CC ) > 0, 'ButtonDownFcn', @ButtonDownFcn, 'UserData', labelmatrix( fig.UserData.CC ) );
    end
    colormap( ax.image, gray );
	axis(ax.image, 'tight' );
    [x, y, ind] = conncomp2xy( fig.UserData.CC, 'PixelIdxListContour' );
    
    %%
    plot( ax.image, x, y, '-r', 'UserData', ind, 'ButtonDownFcn', @ButtonDownFcn, 'DisplayName', 'Contour' );
    ...
    plot( ax.image, fig.UserData.CC.InteriorCentroid(1,tf), fig.UserData.CC.InteriorCentroid(2,tf), '*g', 'UserData', 1:fig.UserData.CC.NumObjects, 'ButtonDownFcn', @ButtonDownFcn, 'DisplayName', 'Interior Centroid', 'Visible', 'off' );
...
switch data.Type
    case 'image'
        fig.UserData.ind(end+1) = data.UserData( round( event.IntersectionPoint(2) ), round(event.IntersectionPoint(1) ) );
    case 'text'
    ...
    case {'axes', 'histogram'}
        ...
        if ~isempty( f )
            ...
            fig.UserData.ind(end+1) = f(i);
        end
    case {'line', 'quiver'}
        [v, m] = min( sum( ( [data.XData; data.YData]' - event.IntersectionPoint([1 2] ) ).^2, 2 ) );
        fig.UserData.ind(end+1) = data.UserData(m);
end
if event.Button == 3
    fig.UserData.ind = fig.UserData.ind(end);
else
    fig.UserData.ind = unique( fig.UserData.ind, 'stable' );
end
update_axes( fig )
...
if fig.UserData.ind > 0
    ...
    BinCenters = mean( hist.BinEdges([1:end-1; 2:end] ) );
    [v, m] = min( ( BinCenters - double( fig.UserData.CC.( ax.histogram.XLabel.String )(fig.UserData.ind) )' ).^2, [], 2 );
    [x, y] = deal( nan( size( fig.UserData.ind ) ) );
    for i=1:numel(m)
        x(i) = interp1( xlim( ax.image ), hist.BinEdges(m(i)+[0 1] ), fig.UserData.CC.Centroid(1,fig.UserData.ind(i) ) );
        y(i) = interp1( ylim( ax.image ), ylim( ax.histogram ), fig.UserData.CC.Centroid(2,fig.UserData.ind(i)) );
    end
    set( focus, 'XData', x, 'YData', y );
end
%%
